...
namespace xBot.App
{
	...
	public partial class Window : Form
	{
		...
		/// </summary>
		public SRSkill[] Skills_GetBuffs(SRMob.Mob type = SRMob.Mob.General)
		{
			SRSkill[] buffs = null;
			switch (type)
			{
				case SRMob.Mob.General:
					Skills_lstvBuffMobType_General.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_General.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_General.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_General.Items[j].Tag;
					});
					break;
				case SRMob.Mob.Champion:
					Skills_lstvBuffMobType_Champion.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_Champion.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_Champion.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_Champion.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.General;
					break;
				case SRMob.Mob.Giant:
					Skills_lstvBuffMobType_Giant.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_Giant.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_Giant.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_Giant.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.Champion;
					break;
				case SRMob.Mob.PartyGeneral:
					Skills_lstvBuffMobType_PartyGeneral.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_PartyGeneral.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_PartyGeneral.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_PartyGeneral.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.Giant;
					break;
				case SRMob.Mob.PartyChampion:
					Skills_lstvBuffMobType_PartyChampion.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_PartyChampion.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_PartyChampion.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_PartyChampion.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.PartyGeneral;
					break;
				case SRMob.Mob.PartyGiant:
					Skills_lstvBuffMobType_PartyGiant.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_PartyGiant.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_PartyGiant.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_PartyGiant.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.PartyChampion;
					break;
				case SRMob.Mob.Unique:
				case SRMob.Mob.Titan:
					Skills_lstvBuffMobType_Unique.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_Unique.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_Unique.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_Unique.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.PartyGiant;
					break;
				case SRMob.Mob.Elite:
				case SRMob.Mob.Strong:
					Skills_lstvBuffMobType_Elite.InvokeIfRequired(() => {
						buffs = new SRSkill[Skills_lstvBuffMobType_Elite.Items.Count];
						for (int j = 0; j < Skills_lstvBuffMobType_Elite.Items.Count; j++)
							buffs[j] = (SRSkill)Skills_lstvBuffMobType_Elite.Items[j].Tag;
					});
					if (buffs.Length == 0)
						goto case SRMob.Mob.Unique;
					break;
