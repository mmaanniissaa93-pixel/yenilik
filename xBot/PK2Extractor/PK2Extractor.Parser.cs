...
namespace xBot.PK2Extractor
{
	partial class Pk2Extractor
	{
		...
		public void AddSkills()
		{
			...
			// short file, load all lines to memory
			string skillIndexFile = "server_dep\\silkroad\\textdata\\SkillDataEnc.txt";
			if (pk2.GetFile(skillIndexFile) == null)
			{
				skillIndexFile = "server_dep\\silkroad\\textdata\\SkillData.txt";
			}
			string skillFileText = pk2.GetFileText(skillIndexFile);
			if (string.IsNullOrEmpty(skillFileText))
			{
				Log("Skill data file not found (" + skillIndexFile + ")");
				return;
			}
