...
internal static class Program
{
    ...
    {
        foreach (ushort error in new ushort[] { 0, 0x0400, 0x05, 0x06, 0x0C, 0x0E, 0xFFFF })
        {
            var openWorld = new World();
            var openRecovery = new CombatObstacleRecovery();
            var openNavigator = openWorld.Navigator();
            bool retryOpen = true;
            for (int i = 0; i < 3; i++)
                retryOpen = openRecovery.OnFailure(true, error,
                    a => openNavigator.TryDetour(new SRCoord(8.0, 0.0), a));
            Check(!retryOpen && openWorld.Commands.Count == 0 && openWorld.Time == 0,
                $"Non-obstacle failure 0x{error:X4} releases target without sideways steps");
        }

        var target = new SRCoord(8.0, 0.0);
        var world = new World();
        Check(world.Navigator().MoveTo(target, 15) && world.Commands.Count == 0,
            "Open-field target in attack range starts without a movement command");
        var clearApproach = world.Navigator();
        Check(clearApproach.MoveTo(target, 3) && !clearApproach.WasBlocked,
            "Unobstructed approach is not diagnosed as a collision");
        Check(world.Commands.Count == 2 && world.Commands[1].DistanceTo(world.Position) < 0.01,
            "Approach sends one destination and stops at attack range without periodic restarts");
        world = new World();
        ...
        Check(recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a)), "First rejection retries without walking");
        Check(world.Commands.Count == 0, "A single skill rejection does not trigger a detour");
        recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a));
        Check(recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a)), "Repeated blocked attacks trigger a detour even in range");
        ...
        recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a));
        recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a));
        Check(recovery.OnFailure(true, 0x10, a => navigator.TryDetour(target, a)) && world.Position.PosY < -2,
        ...
        recovery = new CombatObstacleRecovery();
        int attempts = 0;
        for (int i = 0; i < 2; i++) recovery.OnFailure(true, 0x10, a => false);
        Check(!recovery.OnFailure(true, 0x10, a => { attempts++; return navigator.TryDetour(target, a); }) && attempts == 4,
        ...
        recovery = new CombatObstacleRecovery();
        attempts = 0;
        for (int i = 0; i < 2; i++) recovery.OnFailure(false, 0x10, a => { attempts++; return true; });
        Check(!recovery.OnFailure(false, 0x10, a => { attempts++; return true; }) && attempts == 0,
            "Disabled collision or bypass setting never invokes detour movement");
        recovery = new CombatObstacleRecovery();
        bool retry = true;
        int failures = 0;
        while (retry && failures++ < 100) retry = recovery.OnFailure(true, 0x10, a => true);
        Check(!retry && failures == 15, "Movement without attack success cannot reset the recovery budget forever");
        recovery.OnSuccess();
        Check(recovery.OnFailure(true, 0x10, a => true), "Confirmed attack success resets the failure streak");
