...
- phBot klasik arayüz klon 2. faz: Protection'a `Sockets / Pet Return / Berserk / Monster Preferences / Devil's Spirit / Scrolls / Stats` alt-sekmeleri (md sırasıyla), Training'e `Conditions / Collision / Settings` sekmeleri, Inventory'ye `Exchange / Job Tickets / Item / Gori Item Exchange` sekmeleri eklendi; Trace etiketi ve Savaş→Combat düzeltildi, yanlış `Exchange` etiketi `Avatar` yapıldı, Stall `Consignment` 3. sekme olarak doğrulandı. Gerçek Trade/Quest panelleri ile gerçek Pick Filter sekmeleri sidebar placeholder'larına taşındı (demo içerik yerine backend'e bağlı paneller açılır). Yeni sekmeler önce görsel klondur, backend bağlantıları TODO işaretlidir.

- Quest otomasyonu görev yaşam döngüsüne yükseltildi: her görev için `aktif değilse kabul et`, `bitince teslim et` ve `sunulursa tekrarla` ayrı ayrı ayarlanabilir. Bot görev gerektiğinde şehir dönüşünü başlatır; PK2 `NoticeNPC` alanından potion/blacksmith/storage/grocery NPC'sini veya öğrenilmiş özel NPC konumunu çözüp yürür. Sabit ödüllerin canlı istemcideki 5 baytlık `0x7515 = questId + selection(0)` biçimi ile seçilebilir ödüllerin 9 baytlık biçimi ayrıldı; `rewardId=0` artık hata değildir.

- Quest teslimi gerçek otomasyona dönüştürüldü: etkin görevlerde boş aksiyon eski profiller için `Turn in at NPC`'ye yükseltilir; bot şehirde uygun `QEV_ALL_BASIC_*` görevi bulunca mevcut şehrin potion NPC'sine navmesh ile yürür, NPC'yi seçer, quest konuşmasını açar ve ödülü kendi alır. `refquestrewarditems.txt` PK2 verisi SQLite'a aktarılır; tek ödül doğrudan, çoklu silah ödülü eldeki silah türüyle seçilir ve belirsiz seçim gönderilmez.
- Quest Automation v2 protokol keşif katmanı eklendi: istemcinin gerçek `0x30D4` görev seçimleri ve `0x7515` ödül istekleri otomatik kaydediliyor; sunucunun `0x30D4 QUEST_TALK` cevapları tip 4/5 menü, seçenek, footer ve referans ID alanlarıyla güvenli biçimde ayrıştırılıyor. PK2 görev kataloğuna `NoticeNPC/NoticeCondition` taşındı ve NPC sütunu eklendi. `Select at NPC`, seçili görevin `NameString` değerini açık menüde bulup yalnız kesin eşleşmede istemcinin ürettiği aksiyon kodunu gönderiyor (`SN_` dinamik girdileri için `6`); `0x7515` artık quest ID, seçim ve reward ID olarak çözümleniyor.
- Quest Automation v2'nin ilk dilimi eklendi: PK2 `QuestData.txt` kayıtları çevrilmiş görev adlarıyla SQLite `quests` kataloğuna aktarılıyor. Görevler panelindeki phBot benzeri `Tüm Görevler` alanı ad/kod/ID arama, maksimum seviye filtresi, pasif görevi yapılandırma, temizleme, güncelleme ve otomasyon kurallarını sıfırlama işlevlerini sunuyor. Eski veritabanları bozulmadan açılır; kataloğu kullanmak için PK2 veritabanı bir kez yeniden oluşturulmalıdır.
- Quest Automation v1 tamamlandı: aktif görev başına kalıcı enable/disable, ad/alias, `Do nothing / Return town / Run script` aksiyonu ve script seçimi Görevler paneline eklendi. State 2/8 tamamlanmaları yalnız bir kez kuyruğa alınır ve aksiyonlar network callback yerine bot thread'inde çalıştırılır; karar katmanı 7 yeni regresyon testiyle kapsandı.
- Koyu `ModernSidebar` menüsüne eksik `Trade` ve `Görevler` girişleri eklendi; her iki panel de modern içerik boyutlandırmasına bağlandı.
- İlk görünür Quest paneli eklendi: aktif görevlerin `0x30D5` ile canlı güncellenmesi, seçili görevi `0x70D9/0xB0D9` ile bırakma ve seçili event/So-Ok NPC için `0x30D4/0x3514/0x7515` görev ID/ödül akışı destekleniyor. Normal görev kabulü doğrulanmış bir istemci paketi bulunana kadar açıkça desteklenmiyor.
